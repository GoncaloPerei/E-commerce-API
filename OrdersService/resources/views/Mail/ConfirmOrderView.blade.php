...
<head>
    ...
    @php
    $cartItems = json_decode($order['cart'], true)['cartItem'];
    @endphp
</head>
    ...
    <div class="flex flex-col items-center gap-5 m-8">
        ...
        <div class="w-full flex flex-col gap-3.5">
            ...
            <span class="tracking-wider">
                Hello <span class="font-bold tracking-wider">{{ $order->details['first_name'] }} {{ $order->details['last_name'] }}</span>,
                <br />
                We will send you an email when your order is processed.
            </span>
        </div>
        <table class="w-full">
            <thead>
        ...
        <div class="w-full h-auto flex flex-col gap-2.5">
            ...
            <div class="flex flex-col gap-2.5">
                <span class="tracking-wider">{{ $order->details['first_name'] }} {{ $order->details['last_name'] }}</span>
                <span class="tracking-wider">{{ $order->details['address'] }}</span>
                <span class="tracking-wider">{{ $order->details['city'] }}</span>
                <span class="tracking-wider">{{ $order->details['postal_code'] }}</span>
            </div>
        </div>
